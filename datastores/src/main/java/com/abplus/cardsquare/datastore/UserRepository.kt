...
class UserRepository : User.Repository {
    ...
    override fun findAsync(uid: String): Deferred<User?> = GlobalScope.async {
    ...
    override fun currentAsync(): Deferred<User?> = GlobalScope.async {
        auth.currentUser?.uid?.let {
            findAsync(it).await()
        }
    }

    override fun addAsync(defaultName: String): Deferred<User?> = GlobalScope.async {
        val data = mapOf(
